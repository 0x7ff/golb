			...
			if(lc.cmd == LC_SEGMENT_64) {
				...
				if(mh64.filetype == MH_EXECUTE) {
					if(strncmp(sg64.segname, SEG_TEXT_EXEC, sizeof(sg64.segname)) == 0) {
						...
						if(find_section(p + sizeof(sg64), sg64, SECT_DATA, &s64) != KERN_SUCCESS || s64.size == 0 || (pfinder->sec_data.data = malloc(s64.size)) == NULL || kread_buf(s64.addr, pfinder->sec_data.data, s64.size) != KERN_SUCCESS) {
							break;
						}
...
#endif
			if(pfinder->sec_text.s64.size != 0 && pfinder->sec_data.s64.size != 0 && pfinder->sec_cstring.s64.size != 0) {
				return KERN_SUCCESS;
			}
		}
	}
